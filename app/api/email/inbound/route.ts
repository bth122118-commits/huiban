...
async function applyStatusTransition(
  db: ReturnType<typeof supabase>, task: any, template: any, newIndex: number, note: string
) {
  ...
  await db.from("task_events").insert({
    task_id: task.id, actor_id: null,
    from_status: statuses[task.status_index], to_status: statuses[newIndex], note,
  });
}

export async function POST(req: Request) {
  // TODO(生产)：校验 Resend 签名（Svix：svix-id / svix-timestamp / svix-signature）
  const payload = await req.json();
  const recipient = Array.isArray(payload.to) ? payload.to[0] : payload.to;
  const from = payload.from;
  const subject: string = payload.subject || "";
  const body: string = String(payload.text || "").slice(0, 4000);

  const headers: Record<string, string> = {};
  (payload.headers || []).forEach((h: { name: string; value: string }) => {
  ...
  let templates: any[] = [];
  if (template_id) {
    const { data } = await db.from("templates").select("*").eq("id", template_id).maybeSingle();
    templates = data ? [data] : [];
  } else {
    const { data } = await db.from("templates").select("*").eq("workspace_id", workspace_id);
    templates = data || [];
  }
  ...
  for (const tpl of templates) {
    if (reply) {
      ...
      let task = null;
      for (const r of refs) {
        const { data } = await db.from("tasks").select("*").eq("thread_id", r).eq("template_id", tpl.id).maybeSingle();
        if (data) { task = data; break; }
      }
      if (!task) {
        const bare = subject.replace(/^(re:|回复：|回复:)/i, "").trim();
        const { data } = await db.from("tasks").select("*").ilike("title", `%${bare}%`).eq("template_id", tpl.id).limit(1);
        task = data?.[0] || null;
      }
      if (!task) continue;
      ...
      await db.from("inbox_items").insert({
        workspace_id, template_id: tpl.id, kind: "reply", from_email: from,
        subject, body, matched_keyword: kw, ref_task_id: task.id,
        target_status_index: target, thread_id: inReplyTo || references || null, status: "applied",
      });
    ...
    await db.from("inbox_items").insert({
      workspace_id, template_id: tpl.id, kind: "create", from_email: from,
      subject, body, matched_keyword: kw, thread_id: messageId || null, status: "pending",
    });
